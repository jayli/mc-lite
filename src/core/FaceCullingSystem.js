 ...
 * ## 职责边界
 *
 * 本系统（主线程）与 WorldWorker 分工如下：
 *
 * ### WorldWorker（区块初始化时的批量计算）
 * - 负责区块首次生成时的批量 Face Culling 计算
 * - 负责区块首次生成时的 AO（环境光遮蔽）计算
 * - 计算结果随区块数据一起返回给主线程
 * - 适用于大量方块的初始化场景，避免主线程阻塞
 *
 * ### FaceCullingSystem（主线程，动态增量更新）
 * - 负责玩家放置/移除方块后的增量 Face Culling 更新
 * - 仅计算受影响的方块及其邻居
 * - 不负责区块初始化时的批量计算
 * - 用于实时响应玩家的方块操作
 *
 * ### 数据流
 * 1. 区块生成：WorldWorker 批量计算 → 主线程直接使用结果
 * 2. 动态更新：玩家操作 → FaceCullingSystem 增量计算 → 更新渲染
 * 3. 合并优化：主线程发送数据 → WorldWorker 重新计算 → 替换为优化后的 InstancedMesh
 *
...
import { getFromBlockDataMap } from '../utils/CoordEncoding.js';
...
export class FaceCullingSystem {
  ...
  getBlockFromData(x, y, z, blockData) {
    const type = getFromBlockDataMap(blockData, x, y, z);
  ...
  updateBlock(position, block, neighbors) {
    ...
    try {
      ...
      // 计算可见面状态
      const faceMaskValue = this.calculateFaceVisibility(block, neighbors);
      ...
      if (this.debugMode && this.debugScene) {
        ...
        this.addDebugBlock(blockId, position, faceMaskValue);
      }

      // 更新统计
      const visibleFaces = countVisibleFaces(faceMaskValue);
      ...
      // 触发事件
      this.emit('blockUpdated', { position, block, faceMask: faceMaskValue, visibleFaces, hiddenFaces });
...
// 导入并应用调试和性能监控方法
import { applyFaceCullingSystemDebug } from './FaceCullingSystemDebug.js';
applyFaceCullingSystemDebug(FaceCullingSystem);
